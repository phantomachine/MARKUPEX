...
class baseline_mod(object):
    ...
    def __init__(self, 
              β = 0.9804, σ_CM = 1.0, σ_DM = 0.4225,  
                 ...
                 c = 1.0, τ_b = 0.0, τ_max = 0.075, τgrid_size = 40, 
...
#############################################################################################        
        ...
        #I set the τgrid_size to 40 because my computer can use up to 20 cores.
        #To find out how many cores your system can use, run the following code:
#            import multiprocessing
#            print("Number of CPU cores:", multiprocessing.cpu_count())
#            Number of CPU cores: 20

#      You should set taugrid as a multiple that matches your available cores.

#      For example, if your maximum core count is 15, you can set taugrid to a multiple of 15 (e.g., 15, 30, 45, 60, ...).
...
#############################################################################################  
        ...
        τ_min = β-1.0+1e-2                           # HS: I adjusted it slightly away from the Friedman rule to align it with the graph later on.
...
#     This is the bottom line. It distributes the computation across 20 cores (in my case) via MPI. 
#     After setting up the local configuration as shown below, 
#I    distributed the tasks, then executed the code to gather all results at rank 0 (the main core).        
...
##################################################################################
        ...
        # HS: collect result 
        # For rank 0, I set it to collect the results, while for all other ranks, I set it to None.
        if rank == 0:
...
###############################################################################################            
        #HS: Below is the code that gathers the distributed results using comm.gatherv
...
#### HS: For rank 0, I set it to receive the results, while for all other ranks, I set it to None
...
#######################Custom Calibration Part ###############################

##############################################################################
##############################################################################

# Sam, adjust the parameters here as needed. 
# Then, the money demand fit image file will automatically save according to the specified parameters.
# And based on the specified parameters, the remaining results will be generated accordingly: Markup, M1/GDP, ...
...
model = baseline_mod(σ_DM=σ_DM, Ubar_CM=Ubar_CM, n=n, α_1=α_1)
...
#This part is a bit confusing. I set it to execute only for rank 1. 
#If it's run on all cores, every core ends up computing the entire code. 
# For example, I think it would be better if only rank 0 calculates the SME, then distributes the taugrid across cores for computation. 
# However, it seems that each core is redundantly calculating the full SME and then distributing and computing taugrid, leading to duplication.
...
# As you mentioned in your email, we've updated the dataset to start from 1983. 
# Please remember to adjust the inflation mean, M1/GDP ratio, 
# and any other relevant parameters accordingly to reflect this change."
...
dcut = comm.bcast(dcut, root=0)

# Truncated sample (1983-2007) stats
...
def visualization(x=None, y=None, x_label='T-Bill Rate (3-month, annual)',
                y_label='Markup', model_x=None, model_y=None):
   ...
   if rank == 0:
       data = np.asarray(sorted(zip(x,y)))
       i_set = np.linspace(data[:,0].min(), data[:,0].max(),40)
   ...
   fitted_func = comm.bcast(fitted_func, root=0)
   return fitted_func
...
def L(M1_GDP_target):
   ...
   if rank == 0:    
       ...
       L_diff_min = w_mpy*(diff_M1_GDP)**2.0
       
       # Get results
       M1_GDP_star = result['stat_grid']['mpy_star']
       τ_mean = 0.038
       z_mean, i_mean = model.solve_z_i(z_guess, i_guess, τ_mean)  
       markup_star = model.markup_func(z_mean, i_mean, τ_mean) 
       i_policy_grid = result['stat_grid']['FFR']
 
       #implied parameter values
       σ_DM = model.σ_DM
       σ_CM = model.σ_CM
       α_1 = model.α_1
       Ubar_CM = model.Ubar_CM    
       
       return_values = (M1_GDP_star, markup_star, i_policy_grid, σ_DM, σ_CM, α_1, 
                       Ubar_CM, L_diff_min, model)
   else:
       return_values = [None] * 9
   
   return_values = comm.bcast(return_values, root=0)
   return return_values

if rank == 0:
   # Initial visualization
   M1_GDP_fit_target = visualization(
   ...
   markup_fit_target = visualization(
       x=dcut['T-Bill Rate (3-month, annual)'].tolist(),
...
M1_GDP_target = comm.bcast(M1_GDP_target, root=0)

# Main computation
M1_GDP_star, markup_star, i_policy_grid, σ_DM, σ_CM, α_1, Ubar_CM,\
           L_diff_min, model = L(M1_GDP_target)           

if rank == 0:
   toc = time.time()-tic
   print(f"Computation completed in {toc:.2f} seconds")

   # Results processing and final visualization
   rowlabels = ["$\sigma_{DM}$", "B (Ubar_CM)", "$λ$"]
   model_para = [σ_DM, Ubar_CM, α_1]
   model_para_table = {
       "Calibrated parameters": rowlabels,
       "Model (Avg)": model_para                
   }
   df_model_parameter = pd.DataFrame(data=model_para_table)

   print("\nResults:")
   print(f"Markup: {markup_star}")
   print(f"M1/GDP: {M1_GDP_star}")

   # Final visualization
   M1_GDP_data = visualization(
       x=dcut['T-Bill Rate (3-month, annual)'].tolist(),
       y=dcut['NewM1/GDP'].tolist(),
   ...
   plt.savefig(f"m1_gdp_s{model.σ_DM}_ub{model.Ubar_CM}_n{model.n}_α_1{model.α_1}.png", dpi=800)
   plt.close()
