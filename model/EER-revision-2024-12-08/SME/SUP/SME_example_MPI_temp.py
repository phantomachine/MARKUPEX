...
#            print("💩 You have a problem with your cutoff ordering!")
    ...
    def ρ_min_func(self, z, i, τ):
        ...
        ρ_lb = brentq(vals_obj, self.c, ρ_max, xtol=self.Tol)
    ...
    def solve_z_i(self, z_guess, i_guess, τ):
        ...
        if τ > self.β - 1.0:
            ...
            x = fsolve(self.system_func, x0, xtol=self.Tol, args=(τ), full_output=False)
...
if rank == 0:
    ...
    model = baseline_mod(σ_DM=σ_DM, Ubar_CM=Ubar_CM, n=n, α_1=α_1,τ_max =  τ_max , τgrid_size = τgrid_size, N_reimann=N_reimann,Tol=Tol,β=β)
    z_guess = 0.5
    i_guess = 0.01
    model_hlmw = hlmw_mod(σ_DM=σ_DM, Ubar_CM=Ubar_CM, n=n, 
                         α_1=α_1,τ_max = τ_max, τgrid_size = τgrid_size, 
                         N_reimann=N_reimann,Tol=Tol,β=β)
...
if rank == 0:  # rank 0만 결과 처리 및 시각화 수행
    ...
    plt.legend()
    plt.savefig(f"DW_s{model.σ_DM}_ub{model.Ubar_CM}_n{model.n}_α_1{model.α_1}_N_reimann{model.N_reimann}_tol{model.Tol}.png")
    plt.show()

def gap_percent_func(delta_W_bank, W_no_bank):
    """Use this function to calculate CEV"""
    n_point = τgrid_size
    delta = np.append(np.linspace(2, 1, round(n_point/2), endpoint=False), 
                     (np.linspace(1, 0.1, round(n_point/2))))
...
if rank == 0:
    τ_grid = model.τ_grid 
    τgrid_size = model.τgrid_size
    chunk_size = len(τ_grid) // size
else:
    chunk_size = None
    τ_grid = None 
    τgrid_size = None

τ_grid = comm.bcast(τ_grid, root=0)
τgrid_size = comm.bcast(τgrid_size, root=0)
chunk_size = comm.bcast(chunk_size, root=0)

start_idx = rank * chunk_size
end_idx = start_idx + chunk_size if rank < size-1 else len(τ_grid)

local_Case1_cev = [calculate_cev_percent(j, z_guess, i_guess, case='Case1') 
                   for j in τ_grid[start_idx:end_idx]]

Case1_cev_gathered = comm.gather(local_Case1_cev, root=0)

if rank == 0:
    Case1_cev = []
    for cev_list in Case1_cev_gathered:
        Case1_cev.extend(cev_list)
    Case1_cev = np.array(Case1_cev)

    # Plotting code remains the same but only runs on rank 0
    plt.figure(facecolor='white')
